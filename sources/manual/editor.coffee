jQuery ->
	$ = jQuery
	class ManualEditor extends window.mbgSourceEditor
		$images = null
		$view = null
		$template = null
		constructor: (view) ->
		...
		class ImageEditor
			constructor: (view) ->
				...
				@$lightboxTitle = @$view.find('input.mbg-lightbox-title')
				@$lightboxDescription = @$view.find('textarea.mbg-lightbox-description')
				...
				@mainImageSelector = new mbg.ImageSelector(el: @$view.find('.mbg-manual-main-image'))
				...
				@lightboxImageSelector = new mbg.ImageSelector(el: @$view.find('.mbg-manual-lightbox-image'))
			...
			remove: =>
				...
				renameInputs();
				false
	window.mbgRegisteredSourceEditors.manual = ManualEditor
