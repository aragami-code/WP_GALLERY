...
jQuery(function($) {
  ...
  SourceTabsController = (function(_super) {
    ...
    SourceTabsController.prototype.changeTab = function(event) {
      ...
      $.each(window.mbgSourceEditors, function(name, editor) {
        return editor.hide();
      });
    ...
    SourceTabsController.prototype.createEditor = function(slug) {
      return window.mbgSourceEditors[slug] = new window.mbgRegisteredSourceEditors[slug]($('#source-' + slug + '-settings'));
    };
  ...
  OverlayView = (function(_super) {
    ...
    OverlayView.prototype.el = $('#mbg-overlay');

    OverlayView.prototype.initialize = function() {
      OverlayView.__super__.initialize.apply(this, arguments);
      return new mbg.ImageSelector({
        el: $('#mbg-image-overlay').find('.image-selector')
      });
    };
  ...
  LayoutOptionsView = (function(_super) {
    ...
    LayoutOptionsView.prototype.el = $('#mbg-layout');
    ...
    LayoutOptionsView.prototype.initialize = function() {
      ...
      this.width = this.$('#mbg-image-width');
      this.height = this.$('#mbg-image-height');
      this.select = this.$('#mbg-layout-mode');
      this.hanging = this.$('#mbg-layout-hanging');
  ...
  LoadMoreView = (function(_super) {
    ...
    LoadMoreView.prototype.el = $('#mbg-load-more');
    ...
    LoadMoreView.prototype.initialize = function() {
      ...
      this.loadMoreMode = this.$('#mbg-load-more-mode');
  ...
  CaptionView = (function(_super) {
    ...
    CaptionView.prototype.el = $('#mbg-image-caption');
    ...
    CaptionView.prototype.initialize = function() {
      ...
      _ref = window.mbgGoogleFonts.items;
  ...
  CategoriesView = (function(_super) {
    ...
    CategoriesView.prototype.initialize = function(params) {
      ...
      this.links = this.$el.find('.mbg-tabs li a');
      this.panels = this.$el.find('.mbg-panels li');
      this.links.on('click', this.onLinkClick);
      this.panels.eq(0).addClass('mbg-current');
      return this.links.eq(0).parent().addClass('mbg-current');
    };

    CategoriesView.prototype.onLinkClick = function(event) {
      var index;
      event.preventDefault();
      this.panels.removeClass('mbg-current');
      index = this.links.index(event.target);
      this.panels.eq(index).addClass('mbg-current');
      this.links.parent().removeClass('mbg-current');
      return $(event.target).parent().addClass('mbg-current');
    };
  ...
  return $('#post').submit(function() {
    return $('#mbg-hack').val($('#post').serialize());
  });
});
