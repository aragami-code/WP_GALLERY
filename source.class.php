...
abstract class MBG_Source{
	...
	function fetch_image_size($item, $url, $options){
		$transient_name = "mbg_size" . md5($url);
	...
	function fetch_image_size_uncached($url){
		$response = mbg_remote_get($url, array(
...
class MBG_Http_Source extends MBG_Source{
	...
	function http_get_cached($url, $options = array()){
		return mbg_http_get_cached($url, $options);
	}
	...
	function fetch_raw_images_with_backup($page, $per_page, $state, $options){
		...
		$cached = false;
		if (is_wp_error($data) && defined('MBG_NO_BACKUP') && MBG_NO_BACKUP)
			return array($data, null);
		if (is_wp_error($data)){
			$error = $data;
			$this->send_notification_if_needed($data);
			$cached_data = get_post_meta($this->source['id'], "_mbg-{$this->slug}-cache-" . $page . "-" . $per_page, true);
			$state = get_post_meta($this->source['id'], "_mbg-{$this->slug}-cache-state" . $page . "-" . $per_page,
				true);
			if ($cached_data){
		...
		if (!$cached && (isset($data[0]) && !is_object($data[0]))) {
			update_post_meta($this->source['id'], "_mbg-{$this->slug}-cache-" . $page . "-" . $per_page, str_replace( '\\', '\\\\', json_encode($data)));
			//update_post_meta($this->source['id'], "_mbg_{$this->slug}_cache_" . $page . "_" . $per_page, $data);
			update_post_meta($this->source['id'], "_mbg-{$this->slug}-cache-state" . $page . "-" . $per_page, $state);
	...
	function get_notification_sent_meta_name(){
		return "_mbg_{$this->slug}_notification_sent";
	}
	...
	function send_data_unavailable_notification($error){
		...
		gallery editor page %s", 'mbg'), admin_url("post.php?post={$this->source['id']}&action=edit"));
		$message .= "\r\n\r\nTech details: \r\n";
		$message .= implode("\r\n", $error->get_error_messages());
		@wp_mail(get_option('admin_email'), get_bloginfo('name') . " – " . __('gallery data unavailable', 'mbg'), $message);
	}
	...
	function get_images($options = array()){
		...
		$result = array();
		$page = 1;
		$per_page = min($this->get_max_images(), MBG_MAX_IMAGES);
		do {
			if ($this->sequential)
			...
			$page++;
		} while (count($result) < MBG_MAX_IMAGES && count($data_array));
